...
let selectedAnswers = [];
let currentCategory = "";
let showingResults = false; // true después de finishTest, false al iniciar/reiniciar
...
async function startTest(category){
  ...
  try {
    if (typeof renderOrderToggleAboveQuestion === 'function') renderOrderToggleAboveQuestion(category);
  } catch (err) {
    console.error('Error al renderizar control de aleatoriedad:', err);
  }
}
...
function renderQuestionNav(){
  ...
  currentTest.forEach((_, idx)=>{
    ...
    // limpiar cualquier estilo o clase residual
...
function restartTest(){
  ...
  if (questionNav) {
    Array.from(questionNav.children).forEach(btn=>{
      btn.style.backgroundColor = '';
      btn.classList.remove('correct','incorrect','selected');
    });
    questionNav.innerHTML = '';
